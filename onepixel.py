...
class OnePixel(Attack):
    ...
    def __init__(self, model, pixels=1, steps=10, popsize=10, inf_batch=128, epsilon=1):
    ...
    def forward(self, images, labels):
        ...
        for idx in range(batch_size):
            ...
            adv_image = self._perturb(image, delta, self.epsilon)
    ...
    def _loss(self, image, label, delta):
        adv_images = self._perturb(image, delta, epsilon=self.epsilon)  # Mutiple delta
    ...
    def _attack_success(self, image, label, delta):
        adv_image = self._perturb(image, delta, epsilon=self.epsilon)  # Single delta
    ...
    def _perturb(self, image, delta, epsilon):
